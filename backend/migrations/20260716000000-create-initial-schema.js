...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    // Theatres indexes
    await queryInterface.addIndex('theatres', ['city_id'], { name: 'theatres_city_id_idx' });
    await queryInterface.addIndex('theatres', ['owner_id'], { name: 'theatres_owner_id_idx' });

    // Screens index
    await queryInterface.addIndex('screens', ['theatre_id'], { name: 'screens_theatre_id_idx' });

    // Seats index
    await queryInterface.addIndex('seats', ['screen_id'], { name: 'seats_screen_id_idx' });
    await queryInterface.addIndex('seats', ['screen_id', 'seat_number'], { name: 'seats_screen_id_seat_number_uidx', unique: true });

    // Shows indexes
    await queryInterface.addIndex('shows', ['movie_id'], { name: 'shows_movie_id_idx' });
    await queryInterface.addIndex('shows', ['screen_id'], { name: 'shows_screen_id_idx' });
    await queryInterface.addIndex('shows', ['show_date'], { name: 'shows_show_date_idx' });

    // Bookings indexes
    await queryInterface.addIndex('bookings', ['booking_number'], { name: 'bookings_booking_number_uidx', unique: true });
    await queryInterface.addIndex('bookings', ['user_id'], { name: 'bookings_user_id_idx' });
    await queryInterface.addIndex('bookings', ['show_id'], { name: 'bookings_show_id_idx' });
    await queryInterface.addIndex('bookings', ['event_ticket_id'], { name: 'bookings_event_ticket_id_idx' });

    // Booking Seats indexes
    await queryInterface.addIndex('booking_seats', ['booking_id'], { name: 'booking_seats_booking_id_idx' });
    await queryInterface.addIndex('booking_seats', ['seat_id'], { name: 'booking_seats_seat_id_idx' });

    // Payments indexes
    await queryInterface.addIndex('payments', ['transaction_id'], { name: 'payments_transaction_id_uidx', unique: true });
    await queryInterface.addIndex('payments', ['booking_id'], { name: 'payments_booking_id_idx' });

    // Refunds index
    await queryInterface.addIndex('refunds', ['payment_id'], { name: 'refunds_payment_id_idx' });
    ...
    // Wishlist indexes
    await queryInterface.addIndex('wishlists', ['user_id'], { name: 'wishlists_user_id_idx' });
    await queryInterface.addIndex('wishlists', ['user_id', 'movie_id'], { name: 'wishlists_user_movie_uidx', unique: true, where: { movie_id: { [Sequelize.Op.ne]: null } } });
    ...
    await queryInterface.addIndex('user_refresh_tokens', ['user_id'], { name: 'user_refresh_tokens_user_id_idx' });
  },
